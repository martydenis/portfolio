...
$(document).ready(function(){
   ...
   maskScene = new ScrollMagic.Scene({
         triggerElement: "#projects",
         triggerHook: 0,
         duration: "80%",
      })
      .setTween('#mask-overlay', {opacity: 0, ease: Power2.easeIn})
      .on("progress", function (e) {
         drawCanvasMask(e.progress.toFixed(3))
      })
      .addTo(scrollController);
   ...
   // ART IN MOV
   $('.artinmov_frame_slide').each(function(){      
      let movement = '-' + parseInt(100+(Math.random()*400)) + 'px';
      new ScrollMagic.Scene({
            triggerElement: '#project__artinmov',
            triggerHook: 1,
            duration: '200%',
         })
         .setTween(this, {y: movement})
         .addTo(scrollController);
   });
...
function onResize() {
   ...
   drawCanvasMask(maskScene.progress());
}

var canvas = document.getElementById('mask-canvas'),
    ctx = canvas.getContext('2d');

function drawCanvasMask(progress) {
